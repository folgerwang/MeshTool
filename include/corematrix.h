...
public:
    ...
    matrix3( T a00, T a01, T a02,
             T a10, T a11, T a12,
             T a20, T a21, T a22 ) :
    _11(a00), _12(a01), _13(a02),
    _21(a10), _22(a11), _23(a12),
    _31(a20), _32(a21), _33(a22)
    {}
    ...
    void set_row(int32_t r, const vec3<T> & t) {
        for (int32_t i = 0; i < 3; i++) element(r,i) = t[i];
    }

    void set_column(int32_t c, const vec3<T> & t) {
        for (int32_t i = 0; i < 3; i++) element(i,c) = t[i];
    }

    vec3<T> get_row(int32_t r) const {
        vec3<T> v;
        for (int32_t i = 0; i < 3; i++) v[i] = element(r,i);
        return v;
    }

    vec3<T> get_column(int32_t c) const {
        vec3<T> v;
        for (int32_t i = 0; i < 3; i++) v[i] = element(i,c);
        return v;
    }
    ...
    // dst = src * M
    friend vec3<T> operator *( const vec3<T> &lhs, const matrix3 &rhs) {
        vec3<T> r;
        for ( int32_t i = 0; i < 3; i++)
            r[i]  = ( lhs[0] * rhs.element(0,i) + lhs[1] * rhs.element(1,i) +
                      lhs[2] * rhs.element(2,i));
        return r;
    }
...
////////////////////////////////////////////////////////////
template<class T>
matrix3<T>& rotationX( matrix3<T> & M, const T angle )
{
    T cosa = cos(angle);
    T sina = sin(angle);
...
////////////////////////////////////////////////////////////
template<class T>
matrix3<T>& rotationY( matrix3<T> & M, const T angle )
{
    T cosa = cos(angle);
    T sina = sin(angle);
...
////////////////////////////////////////////////////////////
template<class T>
matrix3<T>& rotationZ( matrix3<T> & M, const T angle )
{
    T cosa = cos(angle);
    T sina = sin(angle);
...
////////////////////////////////////////////////////////////
template<class T>
matrix3<T>& rotationYawPitchRoll( matrix3<T> & M, const T yaw , const T pitch , const T roll )
{
    M.make_identity();
    matrix3<T> rot;
    ...
    if (roll)
    {
        core::rotationZ(M, roll );
    }
    if (pitch)
    {
        M *= core::rotationX(rot, pitch );
    }
    if (yaw)
    {
        M *= core::rotationY(rot, yaw );
    }
